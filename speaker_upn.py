...
def mel_filterbank(nfft, nfiltbank, fs):
    ...
    mel = np.linspace(lower_mel, upper_mel, nfiltbank + 2)  # membuat array vektor dengan nilai dari elemen berjarak nfiltbank+1 dimulai dari lower_mel sampai upper mel
    hertz = [mel_to_hertz(m ) for m in mel] # membuat array vektor dgn elemen berjumlah sama dgn mel dan nilai didapat dari setiap elem mel yang diubah ke hertz
...
def mfcc(s, fs, nfiltbank):
    ...
    # fs adalah nilai frame rate dari data suara, pada program ini bernilai 44100
    # np.int32 berfungsi untuk mengambil nilai dari array dengan tipe int32(-2147483648 to 2147483647)
    nSamples = np.int32(0.025 * fs) # menghitung ukuran frame 
    overlap = np.int32(0.01 * fs)  # menghitung pemisah antar frame     
    nFrames = np.int32(np.ceil(len(s) / (nSamples - overlap))) # np.ceil berfungsi untuk membulatkan nilai dari elemen pada array
    # zero padding to make signal length long enough to have nFrames
    padding = ((nSamples - overlap) * nFrames) - len(s)
    if padding > 0:
        signal = np.append(s, np.zeros(padding)) # np.append untuk menambahkan array s dengan array berjumlah elemen nilai padding dengan nilai 0
    else:
        signal = s
    segment = np.empty((nSamples, nFrames)) # membentuk array baru dengan baris=nSamples dan kolom=nFrames
    start = 0
    for i in range(nFrames):
        segment[:, i] = signal[start:start + nSamples] # mendapatkan nilai sinyal dari masing2 frame
        start = (nSamples - overlap) * i
    # compute periodogram , Mulai FFT
    nfft = 512  # jumlah sample fft dari masing2 frame
    periodogram = np.empty((nFrames, nfft // 2 + 1)) # membentuk array periodogram baru dengan baris=nFrames dan kolom=nfft
    for i in range(nFrames):
        x = segment[:, i] * hamming(nSamples) # hamming = proses windowing hamming window, x merupakan nilai sinyal hasil windowing
        spectrum = fftshift(fft(x, nfft)) # proses fft
        periodogram[i, :] = abs(spectrum[nfft // 2 - 1:]) / nSamples # abs adalah bilangan mutlak
    # calculating mfccs, Mulai mel frekuensi wrapping
    fbank = mel_filterbank(nfft, nfiltbank, fs) # menghitung nilai koeffisien filterbank
...
class pengenal_penutur(QtGui.QMainWindow):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.buttonmulai.clicked.connect(self.mulai)
    ...
    def proses(self):
        ...
        if id >= 1:
            ...
            if jml >= 1:
                ...
                orang = kursor.fetchall()
                for orang in orang:
                    kursor.execute("INSERT INTO masuk (id_penelpon,no_telp,status,id_dosen,waktu,tanggal) VALUES (%s,%s,%s,%s,%s,%s)",
...
class ubah_database_dosen(QtGui.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.labelid.setGeometry(QtCore.QRect(850, 530, 46, 13))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.buttonSimpanUbahDatabaseDosen_3.clicked.connect(self.simpan)
    ...
    def tampildosen(self, pilih):
        ...
        pegawai = kursor.fetchall()
        db.commit()
        self.labelFotoTambahDatabaseDosen.setText(pilih)
        self.labelid.setText(pilih)
        counter = 0
        for pegawai in pegawai:
            counter = 1
    ...
    def simpan(self):
        if self.lineNamaUbahDatabaseDosen.text() != '' and self.lineNIKUbahDatabaseDosen.text() != '' and self.lineJabatanUbahDatabaseDosen.text() != '' and self.lineJurusanUbahDatabaseDosen.text() != '' and  self.lineFakultasUbahDatabaseDosen.text() != '':
            kursor.execute ("UPDATE pegawai SET nama='%s'" % self.lineNamaUbahDatabaseDosen.text() + ",nik='%s'" % self.lineNIKUbahDatabaseDosen.text() + ",jabatan='%s'" % self.lineJabatanUbahDatabaseDosen.text() + ",jurusan='%s'" % self.lineJurusanUbahDatabaseDosen.text() + ",fakultas='%s'" % self.lineFakultasUbahDatabaseDosen.text() + ",no_telp='%s'" % self.lineNomorTeleponUbahDatabaseDosen_2.text() + " WHERE id_dosen='%s'" % self.labelid.text())
            db.commit()
            alertPopup = QtGui.QMessageBox()
            alertPopup.setText("Data berhasil disimpan")
            alertPopup.setIcon(alertPopup.Information)
            alertPopup.exec_()
            if self.lineCariUbahDatabaseDosen.text() != '':
                kursor.execute ("UPDATE foto SET path='%s'" % self.lineCariUbahDatabaseDosen.text() + " WHERE id_dosen='%s'"% self.labelid.text())
                db.commit()  
            self.labelFotoTambahDatabaseDosen.setText("")
            self.lineNamaUbahDatabaseDosen.setText("")
            self.lineNIKUbahDatabaseDosen.setText("")
            self.lineJabatanUbahDatabaseDosen.setText("")
            self.lineJurusanUbahDatabaseDosen.setText("")
            self.lineFakultasUbahDatabaseDosen.setText("")
            self.lineNomorTeleponUbahDatabaseDosen_2.setText("")
            self.lineCariUbahDatabaseDosen.setText("")
            #self.lineRecordTambahDatabaseDosen.setText("")
        else:
            alertPopup = QtGui.QMessageBox()
            alertPopup.setText("Data belum terisi lengkap")
...
class datasuara_penelpon(QtGui.QMainWindow):
    ...
    def kembali(self):
        pilih = self.labeliddosen.text()
        print(pilih)
        self.mainwindow35 = ubah_database_dosen(self)
        self.mainwindow35.tampildosen(pilih)
