...
class home(QtGui.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.retranslateUi(MainWindow)
        self.tampil_penelpon(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)
    ...
    def panggil(self, nCorrect_MFCC):
        timer = QtCore.QTimer(self)
        timer.timeout.connect(self.mulai(nCorrect_MFCC))
        timer.setInterval(2000)
        timer.start() 
    ...
    def refresh(self):
        print("AAAAAA")
        kursor = db.cursor()
        kursor.execute("select * from masuk where id_penelpon = (select max(id_penelpon) from masuk)")
        ...
        if x >= 1:    
            if masuk[0][2] == 'Dikenal':
                kursor = db.cursor()
                kursor.execute("select p.nama, p.nik, m.no_telp, p.jabatan, p.jurusan, p.fakultas, p.foto from masuk m, pegawai p where m.id_dosen = p.id_dosen having max(id_penelpon)")
    ...
    def tampil_penelpon(self, MainWindow):
        kursor = db.cursor()
        kursor.execute("select * from masuk where id_penelpon = (select max(id_penelpon) from masuk)")
        masuk = kursor.fetchall()
        x = len(masuk)
    ...
    def mulai(self, nCorrect_MFCC):
        #for i in range(5):
            ...
            id = len(objek)
            if id >= 1:
                WAVE_OUTPUT_FILENAME = 'C:/Users/asus/workspace/bismillah/test/' + str(id+1) + '.wav'
                ...
                if sp_mfcc >= 1:
                    ...
                    for orang in orang:
                        kursor.execute("INSERT INTO masuk (id_penelpon,no_telp,status,id_dosen,waktu,tanggal) VALUES (%s,%s,%s,%s,%s,%s)",
                            [id+1, '08xxxxx', 'Dikenal', sp_mfcc, datetime.datetime.now().time(), datetime.datetime.now().date()])
                        db.commit()
                        self.refresh()
                elif sp_mfcc == 0:
                    nilai = "Tidak Dikenal"
                ...
                WAVE_OUTPUT_FILENAME = 'C:/Users/asus/Documents/NetBeansProjects/bismillah/src/test/0.wav'
                waveFile = wave.open(WAVE_OUTPUT_FILENAME, 'wb')
                waveFile.setnchannels(CHANNELS)
                waveFile.setsampwidth(audio.get_sample_size(FORMAT))
                waveFile.setframerate(RATE)
                ...
                if sp_mfcc >= 1:
                    ...
                    for orang in orang:
                        kursor.execute("INSERT INTO masuk (id_penelpon,no_telp,status,id_dosen,waktu,tanggal) VALUES (%s,%s,%s,%s,%s,%s)",
                            [str(id+1), '08xxxxx', 'Dikenal', sp_mfcc, datetime.datetime.now().time(), datetime.datetime.now().date()])
                        db.commit()
                        self.refresh()
                elif sp_mfcc == 0:
                    nilai = "Tidak Dikenal"
...
class database_dosen(QtGui.QMainWindow):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.menuHome.triggered.connect(self.tampil_home)
    ...
    def tampil_home(self):
        self.mainwindow5 = home(self)
    ...
    def data_dosen(self, MainWindow):
        kursor = db.cursor()
        kursor.execute("SELECT nama, nik, jurusan, fakultas FROM pegawai")
...
class tambah_database_dosen(QtGui.QMainWindow):
    ...
    def tambah(self):
        if self.lineNamaTambahDatabaseDosen.text() != '' and self.lineNIKTambahDatabaseDosen.text() != '' and self.lineJabatanTambahDatabaseDosen.text() != '' and self.lineJurusanTambahDatabaseDosen.text() != '' and  self.lineFakultasTambahDatabaseDosen.text() != '' and self.lineCariTambahDatabaseDosen.text() != '' and  self.lineRecordTambahDatabaseDosen.text() != '':
                kursor.execute ("INSERT INTO pegawai (id_dosen,nama,nik,jabatan,jurusan,fakultas,file_suara,foto) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
        ...
        for i in range(5):
            ...
            if id >= 1:
                ...
                if sp_mfcc >= 1:
                    ...
                    orang = kursor.fetchall()
                    for orang in orang:
                        kursor.execute("INSERT INTO masuk (nama,nik,no_telp,waktu,tanggal) VALUES (%s,%s,%s,%s,%s)",
